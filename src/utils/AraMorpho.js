...
class AraMorpho extends Morpho {
  ...
  static _gPpOpts() {
    return [
      ...
      {
        person: Morpho.Person.S,
        number: Morpho.GNumber.S,
        gender: Morpho.Gender.M,
      },
      {
        person: Morpho.Person.S,
        number: Morpho.GNumber.S,
        gender: Morpho.Gender.F,
      },
      {
        person: Morpho.Person.S,
        number: Morpho.GNumber.D,
        gender: Morpho.Gender.M,
      },
      {
        person: Morpho.Person.S,
...
function IsriPre32(word) {
  if (word.length >= 6) {
    for (let i in isriP3) if (word.startsWith(isriP3[i])) return word.slice(3);
  }
...
function IsriSuf1(word) {
  for (let i in isriS1) if (word.endsWith(isriS1[i])) return word.slice(0, -1);
  return word;
}

//normalize short prefix
function IsriPre1(word) {
  for (let i in isriP1) if (word.startsWith(isriP1[i])) return word.slice(1);
  return word;
}
...
function __conj(verb, opts) {
  ...
  result += /^[َُِْ]/.test(conjVerb.s)
    ? conjVerb.v.replace(/[َُِْ]$/, "")
    : conjVerb.v;
  result += conjVerb.s;
...
function __affixHandler(conjVerb, opts, pronounIdx) {
  ...
  if (opts.negated) {
    ...
    if (/[^ْ]ن[َِ]$/.test(suffix)) {
      //delete nuun with negation when not preceded by sukuun
...
function __weakBeginHandler(conjVerb, opts) {
  ...
  if (opts.tense === Morpho.Tense.Pr) {
    verb =
      opts.voice === Morpho.Voice.P &&
      len === 3 &&
      !filteredVerb.startsWith("أ")
        ? verb.replace(/^(.)[َُِْ]/, "$1")
        : verb.slice(1);
...
function __weakEndHandler(conjVerb, opts, pronounIdx) {
  ...
  if (opts.voice === Morpho.Voice.P) {
    //
    if (
      opts.tense === Morpho.Tense.Pa ||
      [4, 5, 7, 10, 11, 13].indexOf(pronounIdx) > -1
    ) {
      verb += "ي";
      diacR = "ِ";
    } else verb += "ى";
...
function __diacreticsHandler(conjVerb) {
  ...
  if (diacV) {
    ...
    if (fverb.length < 4 || /^.ا../.test(fverb))
      verb = verb.replace(/^(.)[َُِْ]?/, "$1" + diacV);
    else if (fverb.startsWith("است"))
      verb = verb.replace(/(ا[َُِْ]?س[َُِْ]?ت)[َُِْ]?/, "$1" + diacV);
    else if (len === 5 && /^ا.{3}$/.test(filteredVerb))
      verb = verb.replace(/^(ا?[َُِْ]?.)[َُِْ]?/, "$1" + diacV);
    else if (/ت.{3}/.test(fverb))
      verb = verb.replace(/(.ّ?)[َُِْ]?(.[َُِْ]?)$/, "$1" + diacV + "$2");
    else
      verb = verb.replace(
        /(.*[^َُِْ])[َُِْ]?([^َُِْ][َُِْ]?[^َُِْ][َُِْ]?)$/,
        "$1" + diacV + "$2"
      );
  }
  ...
  if (diacR && !noR) {
    ...
    if (verbInfo.we && fverb.length === 2)
      verb = verb.replace(/^(.[َُِْ]?.)[َُِْ]?/, "$1" + diacR);
    else if (!(verbInfo.m && fverb.length === 2))
      verb = verb.replace(/(.ّ?)[َُِْ]?(.ّ?)[َُِْ]?$/, "$1" + diacR + "$2");
  }
...
function __verbTypes(verb) {
  ...
  //Muda33af
  filteredVerb = /َ?ّ$/.test(verb)
    ? filteredVerb + filteredVerb.slice(-1)
    : filteredVerb;
  //delete diacretics
...
function __conjNormalizeWeakEnd(conjVerb, pronounIdx, opts) {
  ...
  if (tense === Morpho.Tense.Pa) {
    ...
    if ([0, 1, 2, 8, 9].indexOf(pronounIdx) > -1) {
      if (opts.negated && conjVerb.t === Morpho.Tense.Pa)
        verb = verb.slice(0, -1);
      if (
        opts.voice === Morpho.Voice.P ||
        !opts.negated ||
        conjVerb.t === Morpho.Tense.Pa
      )
        suffix = "";
